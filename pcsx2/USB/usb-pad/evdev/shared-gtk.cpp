...
namespace usb_pad
{
	namespace evdev
	{
		...
		static void create_panes(GtkWidget* container, GtkWidget*& left_vbox, GtkWidget*& right_vbox)
		{
			...
			gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(scrolled), right_vbox);
			//gtk_container_add(GTK_CONTAINER(scrolled), right_vbox);
		...
		GtkWidget* make_mappings_treeview(int port, ConfigData& cfg, GtkWidget* container)
		{
			GtkWidget* button;
		...
		int GtkKeyboardmaniaConfigure(int port, const char* dev_type, const char* apititle, const char* apiname, GtkWindow* parent, ApiCallbacks& apicbs)
		{
			...
			for (const auto& it : cfg.joysticks)
			{
				...
				ConfigMapping c;
				c.fd = fd;
				LoadMappings(cfg.dev_type, port, it.id, cfg.max_buttons, 0, c);
				cfg.jsconf.push_back(std::make_pair(it.id, c));
			}
			...
			// ---------------------------
			std::string title = "Keyboardmania ";
			title += apititle;
			...
			if (is_evdev)
			{
				...
				GtkWidget* ro_frame = gtk_frame_new("Device pass-through using hidraw");
				gtk_box_pack_start(GTK_BOX(right_vbox), ro_frame, FALSE, FALSE, 5);
				...
				GtkWidget* rs_cb = new_combobox("Device:", frame_vbox);
